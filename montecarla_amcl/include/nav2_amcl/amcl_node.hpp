...
#include "montecarla_msgs/msg/wifi_scan.hpp"
...
protected:
  ...
  // ── WiFi fingerprinting ────────────────────────────────────────────────────
  void wifiReceived(montecarla_msgs::msg::WifiScan::ConstSharedPtr msg);
  void cargarRadiomaps();
  void aplicarFactorWifi(pf_sample_set_t * set);

  rclcpp::Subscription<montecarla_msgs::msg::WifiScan>::SharedPtr wifi_scan_sub_;

  bool wifi_enabled_{false};
  double wifi_sigma_{8.0};
  double wifi_alpha_{0.5};
  int wifi_min_aps_{1};
  std::string wifi_map_dir_{""};
  ...
  std::unordered_map<std::string, RadiomapInfo> radiomaps_;

  struct MedicionWifi {
    std::vector<std::pair<std::string, float>> mediciones;
  };
  MedicionWifi wifi_cache_;
  std::mutex mutex_wifi_;
  bool wifi_cache_nueva_{false};  // true = hay medición sin consumir; false = ya aplicada
};
