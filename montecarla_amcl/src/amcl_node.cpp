...
: nav2_util::LifecycleNode("amcl", "", options)
{
  ...
  add_parameter("wifi_enabled", rclcpp::ParameterValue(false),
    "Activar fusión WiFi en el filtro de partículas");
  add_parameter("wifi_sigma", rclcpp::ParameterValue(8.0),
    "Desviación típica del modelo de observación WiFi (dBm)");
  add_parameter("wifi_alpha", rclcpp::ParameterValue(0.5),
    "Temperatura de la verosimilitud WiFi (0=apagado, 1=completo)");
  add_parameter("wifi_min_aps", rclcpp::ParameterValue(1),
    "Mínimo de APs válidos para aplicar el factor WiFi");
  add_parameter("wifi_map_dir", rclcpp::ParameterValue(std::string("")),
    "Directorio con radiomap_*.npy y radiomap_meta.yaml");
}
...
AmclNode::laserReceived(sensor_msgs::msg::LaserScan::ConstSharedPtr laser_scan)
{
  ...
  if (lasers_update_[laser_index]) {
    ...
    // Actualizar pesos con WiFi antes del resample
    if (wifi_enabled_) {
      pf_sample_set_t * set_wifi = pf_->sets + pf_->current_set;
      aplicarFactorWifi(set_wifi);
    }
...
AmclNode::initParameters()
{
  ...
  get_parameter("wifi_enabled", wifi_enabled_);
  get_parameter("wifi_sigma", wifi_sigma_);
  get_parameter("wifi_alpha", wifi_alpha_);
  get_parameter("wifi_min_aps", wifi_min_aps_);
  get_parameter("wifi_map_dir", wifi_map_dir_);
...
AmclNode::initPubSub()
{
  ...
  if (wifi_enabled_) {
    wifi_scan_sub_ = create_subscription<montecarla_msgs::msg::WifiScan>(
      "wifi_scan", rclcpp::SensorDataQoS(),
      std::bind(&AmclNode::wifiReceived, this, std::placeholders::_1));
    RCLCPP_INFO(get_logger(), "WiFi habilitado — suscrito a /wifi_scan");
  }
...
// ── WiFi: callback ─────────────────────────────────────────────────────────────
void
AmclNode::wifiReceived(montecarla_msgs::msg::WifiScan::ConstSharedPtr msg)
{
  if (!active_ || !wifi_enabled_) {return;}
  std::lock_guard<std::mutex> lk(mutex_wifi_);
  wifi_cache_.mediciones.clear();
  for (const auto & m : msg->measurements) {
    std::string bssid = m.bssid;
    for (auto & c : bssid) {c = static_cast<char>(std::toupper(c));}
    wifi_cache_.mediciones.push_back({bssid, m.rssi});
  }
  if (!wifi_cache_.mediciones.empty()) {
    wifi_cache_nueva_ = true;  // disponible para el próximo ciclo AMCL
  }
}

// ── WiFi: lector mínimo de .npy (float32, 2D, C-order, versión 1.x) ───────────
static std::vector<float> leerNpy(const std::string & ruta, int & alto, int & ancho)
{
  std::ifstream f(ruta, std::ios::binary);
  if (!f) {throw std::runtime_error("No se puede abrir: " + ruta);}

  // Cabecera mágica (6 bytes) + versión (2 bytes)
  char magic[8];
  f.read(magic, 8);

  // Longitud de la cabecera de texto (2 bytes little-endian en v1)
  uint16_t header_len;
  ...
  // Extraer shape: buscar '(alto, ancho)' o '(alto,ancho)'
  auto pos_s = header.find("'shape'");
  if (pos_s == std::string::npos) {pos_s = header.find("\"shape\"");}
  auto lp = header.find('(', pos_s);
  auto comma = header.find(',', lp);
  auto rp = header.find(')', lp);
  alto  = std::stoi(header.substr(lp + 1, comma - lp - 1));
  ancho = std::stoi(header.substr(comma + 1, rp - comma - 1));

  size_t n = static_cast<size_t>(alto) * static_cast<size_t>(ancho);
  std::vector<float> datos(n);
  f.read(reinterpret_cast<char *>(datos.data()), static_cast<std::streamsize>(n * sizeof(float)));
  return datos;
}

// ── WiFi: carga de radiomaps ───────────────────────────────────────────────────
void
AmclNode::cargarRadiomaps()
{
  if (wifi_map_dir_.empty()) {
    RCLCPP_ERROR(get_logger(), "wifi_map_dir no configurado — WiFi desactivado");
    wifi_enabled_ = false;
    return;
  }

  std::string meta_path = wifi_map_dir_ + "/radiomap_meta.yaml";
  YAML::Node meta;
  try {
    meta = YAML::LoadFile(meta_path);
  } catch (const std::exception & e) {
    RCLCPP_ERROR(get_logger(), "No se puede leer %s: %s", meta_path.c_str(), e.what());
    wifi_enabled_ = false;
    return;
  }

  double resolucion = meta["resolution"].as<double>();
  double origen_x   = meta["origin"][0].as<double>();
  double origen_y   = meta["origin"][1].as<double>();

  for (const auto & node : meta["bssids"]) {
    std::string bssid = node.as<std::string>();
    std::string bssid_up = bssid;
    for (auto & c : bssid_up) {c = static_cast<char>(std::toupper(c));}

    // nombre de fichero: : → _
    std::string nombre = bssid;
    for (auto & c : nombre) {if (c == ':') {c = '_';}}
    std::string ruta = wifi_map_dir_ + "/radiomap_" + nombre + ".npy";

    RadiomapInfo rm;
    try {
      rm.datos = leerNpy(ruta, rm.alto, rm.ancho);
    } catch (const std::exception & e) {
      RCLCPP_WARN(get_logger(), "No se puede cargar %s: %s", ruta.c_str(), e.what());
      continue;
    }
    rm.resolucion = resolucion;
    rm.origen_x   = origen_x;
    rm.origen_y   = origen_y;
    radiomaps_[bssid_up] = std::move(rm);
    RCLCPP_INFO(get_logger(), "Radiomap cargado: %s (%dx%d)", bssid_up.c_str(), rm.ancho, rm.alto);
  }

  if (radiomaps_.empty()) {
    RCLCPP_ERROR(get_logger(), "Ningún radiomap cargado — WiFi desactivado");
    wifi_enabled_ = false;
  } else {
    RCLCPP_INFO(get_logger(), "WiFi listo: %zu radiomaps, sigma=%.1f dBm, alpha=%.2f",
      radiomaps_.size(), wifi_sigma_, wifi_alpha_);
  }
}

// ── WiFi: aplicar factor de verosimilitud a las partículas ────────────────────
void
AmclNode::aplicarFactorWifi(pf_sample_set_t * set)
{
  // Consumir la medición bajo lock: solo se aplica UNA vez por mensaje WiFi.
  // Si no hay medición nueva desde el último ciclo, se omite la actualización.
  MedicionWifi cache;
  {
    std::lock_guard<std::mutex> lk(mutex_wifi_);
    if (!wifi_cache_nueva_) {return;}
    cache = wifi_cache_;
    wifi_cache_nueva_ = false;
  }

  const double dos_sigma2 = 2.0 * wifi_sigma_ * wifi_sigma_;
  ...
  for (int i = 0; i < set->sample_count; i++) {
    ...
    for (const auto & [bssid, rssi_obs] : cache.mediciones) {
      auto it = radiomaps_.find(bssid);
      if (it == radiomaps_.end()) {continue;}

      const RadiomapInfo & rm = it->second;
      int col  = static_cast<int>((x - rm.origen_x) / rm.resolucion);
      int fila = static_cast<int>((y - rm.origen_y) / rm.resolucion);

      if (col < 0 || col >= rm.ancho || fila < 0 || fila >= rm.alto) {continue;}

      float rssi_pred = rm.datos[static_cast<size_t>(fila) * rm.ancho + col];
      if (std::isnan(rssi_pred)) {continue;}

      double residuo = rssi_obs - static_cast<double>(rssi_pred);
      // Sin gating: en simulación no hay outliers de hardware. El gating
      // eliminaba exactamente los pares con residuo alto, es decir, partículas
      // en posición incorrecta — que son las que más necesitamos penalizar.
      log_factor += -(residuo * residuo) / dos_sigma2;
      n_validos++;
    }

    if (n_validos >= wifi_min_aps_) {
      set->samples[i].weight *= std::exp(wifi_alpha_ * log_factor / n_validos);
    }
  }

  // Renormalizar: pf_update_resample computa w_avg = suma/n para el mecanismo
  // de recuperación. Sin normalización, WiFi baja w_avg artificialmente y activa
  // inyección de partículas aleatorias cuando no hay pérdida real de localización.
  double suma = 0.0;
  for (int i = 0; i < set->sample_count; i++) {
    suma += set->samples[i].weight;
  }
  if (suma > 1e-15) {
    const double inv_suma = 1.0 / suma;
    for (int i = 0; i < set->sample_count; i++) {
      set->samples[i].weight *= inv_suma;
    }
  }
}
